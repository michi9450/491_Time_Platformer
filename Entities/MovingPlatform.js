class MovingPlatform {
  ...
  update() {
    ...
    this.velX = this.dirX * this.speed * dir; // used by player as velocity (×TICK applied by player)
    this.velY = this.dirY * this.speed * dir;
    ...
    if (this.movingForward) {
      ...
      const reachedEndX =
        this.dirX > 0 ? this.x >= this.endX : this.x <= this.endX;
      const reachedEndY =
        this.dirY > 0 ? this.y >= this.endY : this.y <= this.endY;

      if (
        (this.dirX !== 0 && reachedEndX) ||
        (this.dirY !== 0 && reachedEndY)
      ) {
        this.x = this.endX;
        this.y = this.endY;
      ...
      const reachedStartX =
        this.dirX > 0 ? this.x <= this.startX : this.x >= this.startX;
      const reachedStartY =
        this.dirY > 0 ? this.y <= this.startY : this.y >= this.startY;

      if (
        (this.dirX !== 0 && reachedStartX) ||
        (this.dirY !== 0 && reachedStartY)
      ) {
        this.x = this.startX;
        this.y = this.startY;
        this.movingForward = true;
